...
import com.sap.bulletinboard.ads.models.Advertiser;
import com.sap.bulletinboard.ads.models.AdvertiserRepository;
...
public class AdvertisementController {
    ...
    public AdvertisementController(AdvertisementRepository repository, ReviewsServiceClient reviewsServiceClient, AdvertiserRepository advertiserRepository) {
    ...
    @KafkaListener(topics = "bulletinboard", groupId = "averagerating-group")
    public void receiveMessage(String message) {
        String[] arrOfStr = message.split(" ", 2);
        for (String a : arrOfStr)
            System.out.println(a);
        Advertiser user = advertiserRepository.findByEmail(arrOfStr[0]);
        if(user == null) {
            Advertiser advertiser = new Advertiser(arrOfStr[0], Double.parseDouble(arrOfStr[0]));
            advertiserRepository.save(advertiser);
        }
        else{
            user.setAverageRating(Double.parseDouble(arrOfStr[1]));
    ...
    private AdvertisementDto entityToDto(Advertisement ad) {
        ...
        Advertiser advertiser = advertiserRepository.findByEmail(dto.contact);
        Double averageRating = 1D;
        if(advertiser != null) {
            averageRating = advertiser.getAverageRating();
        }
